...
- 软件行业
	...
	- **Code Llama**：代码生成大语言模型
		- 在通用文本和代码数据上预训练的基础性大模型
		- 第一步是在 **5000 亿词元的公开代码数据集**上训练出一个模型。该数据集中除了有**代码数据集**外，还包含一些**自然语言数据集**，例如**有关代码和代码片段的讨论**，且最终数据集是使用近似去重法去过重的。不幸的是，Meta 没有披露有关该数据集的更多信息。
		- 在对模型进行指令微调时，使用了两个数据集: 为 **Llama 2 Chat 收集的指令微调数据集**和**自指令数据集**。自指令数据集收集了 Llama 2 编制出的编程面试问题，然后使用 Code Llama 生成单元测试和解答，最后通过执行测试来评估解答。
