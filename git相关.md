...
- 将本地的master分支和远程next分支进行关联
	...
	- 在某些场合，Git会自动在本地分支与远程分支之间，建立一种追踪关系(tracking)。比如，在git clone的时候，本地分支默认与远程主机的同名分支，建立追踪关系，也就是说，本地的master分支自动”追踪”origin/master分支。建立关联的好处在于git pull和push的时候可以自动识别
